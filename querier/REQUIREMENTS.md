...
The specs above indicate that querier reads one query per line, parses the query according to its *syntax*, determines which documents *satisfy* the query, and determine *score* for each document satisfying the query.
...
First, querier ignores blank lines (i.e., empty or nothing but spaces).

Second, it translates all upper-case letters on the input line into lower-case.

Then, it uses [Backus-Naur Form](https://en.wikipedia.org/wiki/Backus–Naur_Form) for describing the query syntax; if you have not heard of BNF before, you're sure to see it in future CS classes (or software specs!).
...
Querier converts the query as a sequence of *tokens*, each a `word` or a literal ('and', 'or'), with spaces between tokens, and optional spaces at the beginning or end of the line.
The literal 'and' may be left out and is thus implicit.

The structure of the syntax indicates that 'and' has higher precedence (binds more tightly) than 'or'.
...
One can combine words more richly, of course:
...
Example output from the querier for some of the queries above, on a depth-3 index of the 'toscrape' section of the testing site:
...
To rank the documents satisfying the query, querier scores each document by *how well* it satisfies the document.
...
For example, considering three documents (D1, D2, D3) and three queries:
